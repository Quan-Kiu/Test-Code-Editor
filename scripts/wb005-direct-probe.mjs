...
const url = process.env.REAL_SERVER_URL || 'http://127.0.0.1:4188';
const report = { story: 'WB-005 direct localhost probe', browserMode: 'headed Chromium + Xvfb', url, localhostDirect: 'running', bridgeUsed: false, appSource: 'real Vite preview server', consoleErrors: [], pageErrors: [], result: 'running' };
...
} finally { await writeFile('docs/validation-reports/wb-005-direct-probe.json', JSON.stringify(report, null, 2)); await browser.close().catch(() => undefined); }
